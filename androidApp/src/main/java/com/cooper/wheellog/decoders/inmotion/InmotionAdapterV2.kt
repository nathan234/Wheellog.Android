...
import com.cooper.wheellog.decoders.BaseAdapter
...
class InmotionAdapterV2 : BaseAdapter() {
    ...
    private var settingCommand: ByteArray? = null
                ...
                if (result != null) {
                    ...
                    if (result.flags == Message.Flag.Initial.value) {
                        if (result.command == Message.Command.MainInfo.value) {
                            ...
                            result.command == Message.Command.Diagnistic.value && turningOff
                        ) {
    ...
    class Message {
        ...
        fun getError(i: Int): String {
            ...
            if (data[i + 3].toInt() shr 2 and 0x03 > 0)
                inmoError +=
                    "err_overBusCurrentState-" + (data[43].toInt() shr 2 and 0x03).toString() + " "
            if (data[i + 3].toInt() shr 4 and 0x03 > 0)
                inmoError +=
                    "err_lowBatteryState-" + (data[43].toInt() shr 4 and 0x03).toString() + " "
        ...
        fun parseRealTimeInfoV11(sContext: Context?): Boolean {
            ...
            // %.2f, N*m: %.2f, Bat Wt: %d, Mot Wt: %d, PWM: %.2f, PitchAim: %.2f, Pith: %.2f, Roll:
            // %.2f, \nTrip Km: %.2f, Rem Km: %.3f, Bat: %.2f, Lim km/h: %.2f, Lim A: %.2f, \nMos t:
            // %d, Mot t: %d, Bat t: %d, Board t: %d, CPU t: %d, IMU t: %d, Lamp t: %d",
            //                    mVoltage/100.0, mCurrent/100.0, mSpeed/100.0, mTorque/100.0,
            // mBatPower,mMotPower, mPwm/100.0, mPitchAimAngle/100.0, mPitchAngle/100.0,
            // mRollAngle/100.0, mMileage/10.0, mRemainMileage/1000.0, mBatLevel/100.0,
            // mDynamicSpeedLimit/100.0, mDynamicCurrentLimit/100.0, mMosTemp, mMotTemp, mBatTemp,
            // mBoardTemp, mCpuTemp, mImuTemp, mLampTemp));

            //            if (!(wmode.equals("Active") || wmode.equals("")))
            // System.out.println(String.format(Locale.US,"State: %s", wmode));
            //            if (!inmoError.equals(""))
            // System.out.println(String.format(Locale.US,"Err: %s", inmoError));
            return true
        }
        ...
        fun parseRealTimeInfoV11_1_4(sContext: Context?): Boolean {
            ...
            //            System.out.println(String.format(Locale.US,"\nVolt: %.2f, Amp: %.2f, Km/h:
            // %.2f, N*m: %.2f, Bat Wt: %d, Mot Wt: %d, XZ: %d, PWM: %.2f, PitchAim: %.2f, Pith:
            // %.2f, Roll: %.2f, \nTrip Km: %.2f, Rem Km: %.3f, Bat: %.2f, Something: %.2f, Lim
            // km/h: %.2f, Lim A: %.2f, \nMos t: %d, Mot t: %d, Bat t: %d, Board t: %d, CPU t: %d,
            // IMU t: %d, Lamp t: %d",
            //                    mVoltage/100.0, mCurrent/100.0, mSpeed/100.0, mTorque/100.0,
            // mBatPower,mMotPower, mXz, mPwm/100.0, mPitchAimAngle/100.0, mPitchAngle/100.0,
            // mRollAngle/100.0, mMileage/10.0, mRemainMileage/1000.0, mBatLevel/100.0,
            // mSomeThing180/100.0, mDynamicSpeedLimit/100.0, mDynamicCurrentLimit/100.0, mMosTemp,
            // mMotTemp, mBatTemp, mBoardTemp, mCpuTemp, mImuTemp, mLampTemp));
            wd.voltage = mVoltage
            wd.torque = mTorque.toDouble() / 100.0
            wd.motorPower = mMotPower.toDouble()
            wd.cpuTemp = mCpuTemp
        ...
        companion object {
            ...
            fun verify(buffer: ByteArray): Message? {
                Timber.i("Verify: %s", toHexString(buffer))
