...
class NinebotAdapter(
    ...
    fun startKeepAliveTimer(protoVer: String) {
        ...
        val timerTask: TimerTask = object : TimerTask() {
            override fun run() {
                if (updateStep == 0) {
                    if (stateCon == 0) {
                        if (wd.bluetoothCmd(CANMessage.serialNumber.writeBuffer())) {
                            Timber.i("Sent serial number message")
                        } else updateStep = 39
                    } else if (stateCon == 1) {
                        if (wd.bluetoothCmd(CANMessage.version.writeBuffer())) {
                        ...
                        if (!wd.bluetoothCmd(CANMessage.liveData.writeBuffer())) {
    ...
    fun charUpdated(data: ByteArray?): ArrayList<Status> {
        ...
        Timber.i("Got data ")
        for (c in data!!) {
            val (first, second) = unpacker.addChar(c.toInt(), updateStep)
            updateStep = second
            if (first) {
                Timber.i("Starting verification")
                val result = CANMessage.verify(
                    unpacker.getBuffer()
                )
