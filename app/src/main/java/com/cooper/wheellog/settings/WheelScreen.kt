...
private fun ninebotZ() {
    ...
    if (NinebotZAdapter.instance?.getLedIsAvailable(1) == true) {
        sliderPref(
    ...
    if (NinebotZAdapter.instance?.getLedIsAvailable(2) == true) {
        sliderPref(
    ...
    if (NinebotZAdapter.instance?.getLedIsAvailable(3) == true) {
        sliderPref(
    ...
    if (NinebotZAdapter.instance?.getLedIsAvailable(4) == true) {
        sliderPref(
...
private fun kingsong() {
    val adapter by remember { mutableStateOf(KingsongAdapter.instance!!) }
    var speedMultipier = 1.0f
    var speedUnit = R.string.kmh
    if (AppConfig.useMph) {
        speedMultipier = MathsUtil.kmToMilesMultiplier.toFloat()
